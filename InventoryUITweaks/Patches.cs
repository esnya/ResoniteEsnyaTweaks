...
{
    [System.Diagnostics.CodeAnalysis.SuppressMessage("CodeQuality", "IDE0051:Remove unused private members", Justification = "Harmony patch method")]
    [System.Diagnostics.CodeAnalysis.SuppressMessage("Style", "IDE0060:Remove unused parameter", Justification = "Harmony patch signature")]
...
internal static class InventoryBrowser_ProcessItem_Postfix
{
    ...
    private static void Postfix(InventoryItemUI item)
    {
        ...
        bool isFavorite = item.NormalColor.Value == InventoryBrowser.FAVORITE_COLOR;
        ...
        if (isDirectory)
        {
            ...
            if (slot.FindChild(HelperName) == null)
            {
                ...
                float itemSize = inv?.ItemSize.Value ?? BrowserDialog.DEFAULT_ITEM_SIZE;
                float pad = BrowserDialog.PADDING;
                // Target ~50% of folder button height, with a small inset
                float folderHeight = 0.5f * itemSize + pad;
                float side = folderHeight * 0.5f - pad * 0.25f;
                if (side < 20f) side = 20f;
                // Place square at left with small padding
                rt.AnchorMin.Value = new float2(0.0f, 0.5f);
                rt.AnchorMax.Value = new float2(0.0f, 0.5f);
                float leftPad = pad * 2f;
...
internal static class InventoryBrowser_OnItemSelected_Postfix
{
    ...
    private static void Postfix(InventoryBrowser __instance, BrowserItem previousItem, BrowserItem currentItem)
    {
        ...
        foreach (var it in items)
        {
            ...
            if (helper == null)
                continue;
            bool isSelected = ReferenceEquals(it, selected);
...
internal static class InventoryBrowser_BeginGeneratingNewDirectory_Patch
{
    static MethodBase TargetMethod() => AccessTools.Method(
        ...
        new[] { typeof(RecordDirectory), typeof(GridLayout).MakeByRefType(), typeof(GridLayout).MakeByRefType(), typeof(SlideSwapRegion.Slide) }
    );
    [HarmonyPostfix]
